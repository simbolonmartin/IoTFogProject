...
clients=[
{"broker":"192.168.50.99","port":1883,"name":"blank","sub_topic":"Temp","pub_topic":"test1"},
{"broker":"192.168.50.99","port":1883,"name":"blank","sub_topic":"EMG","pub_topic":"test2"}
]
...
def on_message(client, userdata, message):
   ...
   elif(message.topic=='EMG'):
       out_queue_emg.append(message.payload.decode("utf-8"))
...
try:
   while Run_Flag:
      ...
      for x in range(len(out_queue)):
         ...
         out_temp=float(out_queue.pop())
         out_emg=float(out_queue_emg.pop())
         
         _ThingSpeeak.send(out_emg,out_temp)
      count+=1
      #time.sleep(5)#wait
